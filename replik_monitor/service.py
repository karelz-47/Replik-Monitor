...
def poll_once(client, repository, settings, now: datetime | None = None) -> dict:
    ...
    # getKonaniePodlaICO pages the complete company-specific reconciliation. Unlike
    # the global feed it is not a capped pre-filter and total >500 is valid.
    fetched = client.fetch_changes(COMPANY_ICO, since, settings.historical_batch_limit)
    changes = fetched.changes
    changes.sort(key=lambda item: (item.changed_at, item.source_id, item.change_marker))
    if settings.dry_run:
    ...
    if initial:
        ...
        mode = "incremental-proceeding-state"
