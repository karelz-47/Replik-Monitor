...
class Settings:
    ...
    def from_env(cls) -> "Settings":
        required = ("DATABASE_URL", "REPLIK_ENDPOINT", "RESEND_API_KEY", "RESEND_FROM", "ALERT_TO", "MONITOR_EXPIRES_AT", "MONITOR_HISTORICAL_SINCE")
        ...
        return cls(
            database_url=os.environ["DATABASE_URL"], replik_endpoint=os.environ["REPLIK_ENDPOINT"],
            replik_action=os.getenv("REPLIK_SOAP_ACTION", ""), resend_api_key=os.environ["RESEND_API_KEY"],
            resend_from=os.environ["RESEND_FROM"], alert_to=os.environ["ALERT_TO"], expires_at=expires_at,
            historical_since=parse_utc(os.environ["MONITOR_HISTORICAL_SINCE"]), historical_batch_limit=limit,
            poll_overlap_minutes=overlap, stale_after_minutes=stale,
